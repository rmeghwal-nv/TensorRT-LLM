...
class MarlinFusedMoE(CutlassFusedMoE):
    ...
    def run_moe(
        ...
        *,
        ...
        # Step 5: sum weighted expert outputs.
        # gemm2_out rows correspond to flattened (token_idx * top_k + k) pairs.
        # The Triton path fuses top_k iterations into one launch over the
        # contiguous layout; the ATen fallback uses index_add_ scatter-reduce.
        gemm2_out = gemm2_out[:num_tokens_gemm2, : self.unpadded_hidden_size]

        if allow_triton_moe_sum_topk() and gemm2_out.is_contiguous():
